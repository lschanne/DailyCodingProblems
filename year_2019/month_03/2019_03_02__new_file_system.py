...
subdir2 contains a second-level sub-directory subsubdir2 containing a
file file2.ext.
...
def get_longest_path(dir_string):
    regex = re.compile(r'[\w\.]+|\s+')
    longest_path = ''
    dirs = []
    for x in re.findall(regex, dir_string):
        if x.startswith('\n'):
            depth = x.count('\t')
            if depth < len(dirs):
                dirs.pop()
        elif '.' in x:
            path = os.path.join(os.sep.join(dirs), x)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        dir_string = 'dir\n\tsubdir1\n\t\tfile1.ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext'
    longest_path = get_longest_path(dir_string)
